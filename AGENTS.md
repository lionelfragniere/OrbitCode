...
- 2026-05-16: Renamed OverGravity to OrbitCode, removed legacy corporate branding, added beginner mode, introduced pluggable provider architecture, added MemPalace-backed memory service, moved storage to `.orbitcode` with legacy `.overgravity` fallback, and replaced the launcher with a local-first single-file Windows script.
